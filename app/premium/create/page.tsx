  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (projectType === 'single') {
      ...
      for (let i = 0; i < albumSongs.length; i++) {
        ...
        // Add artwork_url if we have it (stored as JSONB or text field)
        if (artworkUrl) {
          insertPayload.artwork_url = artworkUrl
          console.log(`✅ Adding artwork_url to insert payload for Song ${i + 1}:`, artworkUrl)
        }
        ...
        // If insert failed because of album columns, try workaround
        if (songError) {
          console.log(`Insert with album fields failed, trying workaround for Song ${i + 1}:`, songError.message)
          
          // Insert without album fields first - BUT INCLUDE demo_mp3!
          ...
          // CRITICAL: Add demo_mp3 to base payload too!
          if (demoUrl) {
            basePayload.demo_mp3 = demoUrl
            console.log(`✅ Adding demo_mp3 to base payload for Song ${i + 1}:`, demoUrl)
          }
          
          // Add artwork_url to base payload
          if (artworkUrl) {
            basePayload.artwork_url = artworkUrl
            console.log(`✅ Adding artwork_url to base payload for Song ${i + 1}:`, artworkUrl)
          }
          ...
          if (!updatedSong.album_id || updatedSong.album_id !== albumId) {
            console.error(`ALBUM_ID NOT SAVED! Expected: ${albumId}, Got: ${updatedSong.album_id}`)
          ...
          if (demoUrl && !updatedSong.demo_mp3) {
            console.error(`⚠️ DEMO_MP3 NOT SAVED! Expected: ${demoUrl}, Got: ${updatedSong.demo_mp3}`)
            // Don't fail - just warn, demo is optional
          } else if (demoUrl && updatedSong.demo_mp3) {
            console.log(`✅ demo_mp3 verified saved for Song ${i + 1}:`, updatedSong.demo_mp3)
          }
  ...
  return (
    <main style={{
      ...
      <div style={{ width: '100%', maxWidth: '600px' }}>
        <h1 style={{ fontSize: '3rem', fontWeight: 'bold', marginBottom: '2rem' }}>PREMIUM CREATE</h1>
            ...
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
              <button
                ...
                style={{
                  ...
                  backgroundColor: projectType === 'single' ? '#B8001F' : 'white',
                  color: projectType === 'single' ? 'white' : '#B8001F',
              ...
              <button
                ...
                style={{
                  padding: '0.75rem 2rem',
                  backgroundColor: projectType === 'album' ? '#B8001F' : 'white',
                  color: projectType === 'album' ? 'white' : '#B8001F',
                  border: '2px solid #B8001F',
                  cursor: 'pointer',
                  fontSize: '1rem',
                  fontWeight: 'bold',
                ...
                <div style={{ position: 'relative', width: '100%' }} data-dropdown>
                  ...
                  {showFileSourceDropdown && (
                    <div style={{
                      ...
                      <div onClick={() => { setFileSource('device'); setShowFileSourceDropdown(false); }} style={{ padding: '0.5rem', cursor: 'pointer', backgroundColor: fileSource === 'device' ? '#f3f3f3' : 'white' }}>
                        📱 From This Device
                      </div>
                      <div onClick={() => { setFileSource('dropbox'); setShowFileSourceDropdown(false); }} style={{ padding: '0.5rem', cursor: 'pointer', backgroundColor: fileSource === 'dropbox' ? '#f3f3f3' : 'white' }}>
                        📁 From Dropbox
                      </div>
                    </div>
                  )}
                    ...
                    <div key={i} style={{ marginBottom: '0.5rem' }}>
                      {editingIndex === i ? (
                        <input
                          ...
                          style={{ width: '100%', padding: '0.4rem', backgroundColor: 'white', color: 'black', border: '1px solid #ccc' }}
                        />
                      ) : (
                ...
                {albumSongs.map((song, index) => (
                  <div key={index} style={{ border: '2px solid #B8001F', borderRadius: '8px', padding: '1.5rem', backgroundColor: 'white' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                      <h3 style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#B8001F' }}>SONG {index + 1}</h3>
                      {albumSongs.length > 1 && (
                        <button
                          type="button"
