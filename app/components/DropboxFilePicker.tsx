...
export default function DropboxFilePicker({ onFilesSelected, isMobile }: DropboxFilePickerProps) {
  ...
  useEffect(() => {
    ...
    const initDropbox = () => {
      ...
      if (existingScript) {
        ...
        const checkInterval = setInterval(() => {
          ...
          if (window.Dropbox && typeof window.Dropbox.choose === 'function') {
            ...
            setError(`Dropbox not initializing. Domain "${currentDomain}" must be registered in Dropbox App Console → Settings → "Chooser / Saver / Embedder domains".`)
            clearInterval(checkInterval)
          }
        }, 200)
      ...
      // Load the Dropbox script manually
      ...
      script.onload = () => {
        ...
        const checkInterval = setInterval(() => {
          ...
          if (window.Dropbox && typeof window.Dropbox.choose === 'function') {
            ...
            const currentDomain = window.location.hostname
            setError(`Dropbox API not initializing. Domain "${currentDomain}" must be registered in Dropbox App Console → Settings → "Chooser / Saver / Embedder domains". Add exactly "${currentDomain}" (no www, no protocol).`)
            clearInterval(checkInterval)
          }
        }, 200)
      }
      ...
      window.Dropbox.choose({
        ...
        error: (errorMessage: string) => {
          ...
          if (errorMessage.includes('misconfigured') || errorMessage.includes('not configured') || errorMessage.includes('Could not communicate')) {
            setError(`Domain "${currentDomain}" must be registered in Dropbox App Console → Settings → "Chooser / Saver / Embedder domains". Add exactly "${currentDomain}" (no www, no protocol). Wait 2-3 minutes after adding.`)
          } else {
