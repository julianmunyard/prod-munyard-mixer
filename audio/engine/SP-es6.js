...
class LinearMemoryBuffer {
  ...
  constructor(
    ...
    this.pointer =
      pointer == 0
        ? glue.malloc(
            length * LinearMemoryBuffer._types[this._type].BYTES_PER_ELEMENT
          )
        : pointer;
    this.update();
  ...
  update() {
    const ab = this._glue.linearMemory,
      t = LinearMemoryBuffer._types[this._type];
    this.array = new t(
      ab,
      this.pointer,
      this.length < 0
        ? Math.floor((ab.byteLength - this.pointer) / t.BYTES_PER_ELEMENT)
        : this.length
    );
  }
...
class SuperpoweredGlue {
  ...
  /**@type {Map<string,Map<string,Function>>}*/ _functionsWithNamespace =
    new Map();
  ...
  /**@type {DataView} */ _view;
  /**@type {boolean} */ _littleEndian =
    new Uint8Array(new Uint32Array([0x11223344]).buffer)[0] === 0x44;

  /**@returns {Promise<SuperpoweredGlue>} */
  static async Instantiate(
    /**@type {string}*/ licenseKey,
    /**@type {string}*/ wasmUrl = SuperpoweredGlue.wasmCDNUrl,
    /**@type {boolean}*/ sharedArrayBuffer = false
  ) {
    ...
    await obj.loadFromArrayBuffer(
      sharedArrayBuffer
        ? SuperpoweredGlue.getWASMWithSharedArrayBufferEnabled(ab)
        : ab
    );
  ...
  async loadFromModule(/**@type {BufferSource}*/ module) {
    await WebAssembly.instantiate(module, {
      wasi_snapshot_preview1: this.wasi,
      env: this._exportsToWASM,
    }).then((result) => this.setInstance(result.instance));
  }

  async loadFromURL(
    /**@type {string}*/ url,
    /**@type {boolean}*/ storeCode = true
  ) {
    const wasmCode = await fetch(url).then((response) =>
  ...
  static getWASMWithSharedArrayBufferEnabled(/**@type {ArrayBuffer} */ wasm) {
    const v = new DataView(wasm),
      to = wasm.byteLength,
      result = new Uint8Array(wasm.byteLength);
  ...
  constructor() {
    ...
    this.Uint8Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 1, glue);
      }
    };
    this.Int8Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 2, glue);
      }
    };
    this.Uint16Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 3, glue);
      }
    };
    this.Int16Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 4, glue);
      }
    };
    this.Uint32Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 5, glue);
      }
    };
    this.Int32Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 6, glue);
      }
    };
    this.BigUint64Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 7, glue);
      }
    };
    this.BigInt64Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 8, glue);
      }
    };
    this.Float32Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 9, glue);
      }
    };
    this.Float64Buffer = class {
      constructor(/**@type {number}*/ length) {
        return new LinearMemoryBuffer(0, length, 10, glue);
      }
    };
    this._exportsToWASM = {
      consolelog: (/**@type {number}*/ pointer, /**@type {number}*/ strlen) =>
      ...
      __createClassConstant__: (
        /**@type {number}*/ nameptr,
        /**@type {number}*/ namelen,
        /**@type {number}*/ value
      ) =>
        (this._classUnderConstruction[this.toString(nameptr, namelen)] = value),
      __createConstant__: (
        /**@type {number}*/ nameptr,
        /**@type {number}*/ namelen,
        /**@type {number}*/ value
      ) => (this[this.toString(nameptr, namelen)] = value),
      __runjs__: (/**@type {number}*/ pointer) => {
  ...
  setInstance(/**@type {WebAssembly.Instance}*/ wasmInstance) {
    ...
    this._lastArrayLength = /**@type {Function}*/ (
      this._wasmInstance.exports.__lastarraylength__
    );
    this._malloc = /**@type {Function}*/ (
      this._wasmInstance.exports.__malloc__
    );
    this._stackSize = /**@type {Function}*/ (
      this._wasmInstance.exports.__stacksize__
    );
    this._heapBase = /**@type {Function}*/ (
      this._wasmInstance.exports.__heapbase__
    );
    this._free = /**@type {Function}*/ (this._wasmInstance.exports.__free__);
    this._setInt64 = /**@type {Function}*/ (
      this._wasmInstance.exports.__setint64__
    );
    ...
    this._memoryGrowArray = new Uint8Array(
      this.linearMemory,
      this._memoryGrowPointer,
      16
    );

    const outputBuffer = this._malloc(1024),
      stringview = new Uint8Array(this.linearMemory, this._malloc(1024), 1024),
      demangle = /**@type {Function}*/ (
        this._wasmInstance.exports.__demangle__
      );
    for (const name in this._wasmInstance.exports)
      if (name != "__demangle__") {
        const length = demangle(
            this.toWASMString(name, stringview),
            outputBuffer
          ),
          func = /**@type {Function}*/ (this._wasmInstance.exports[name]);
        if (length > 0) {
          ...
          if (namespace > 0) {
            ...
            if (namespace > 0)
              demangledName = demangledName.substr(namespace + 2);
          }
  ...
  _invokeFunction(
    ...
    /**@type {Function} */ func,
    /**@type {number} */ returnPointerType
  ) {
    if (arguments.length == 4 && typeof arguments[3] == "object") {
      const obj = arguments[3];
      let n = 0;
    ...
    for (let index = 3; index < to; index++) {
      if (arguments[index].array != undefined)
        args.push(arguments[index].array.byteOffset);
      else if (arguments[index].pointerToInstance != undefined)
        args.push(arguments[index].pointerToInstance);
  ...
  _createClass(
    /**@type {number}*/ classnamePointer,
    /**@type {number}*/ classnameLen,
    /**@type {number}*/ sizeofClass
  ) {
    const classname = this.toString(classnamePointer, classnameLen),
      glue = this,
      O = class {
        ...
        constructor() {
          const constructorFunction = glue[classname + "::" + classname],
            args = [].slice.call(arguments);
          if (constructorFunction == undefined)
            throw classname + " has no constructor";
          else args.unshift(glue.malloc(sizeofClass));
          ...
          for (const property of meta.properties)
            glue.createPropertyFromDescriptor(this, property);
          for (const method of meta.methods)
            this[method.name] = glue._invokeFunction.bind(
              glue,
              this.pointerToInstance,
              method.function,
              method.returnPointerType
            );
        }
  ...
  _read(/**@type {number} */ pointer, /**@type {number} */ type) {
    switch (type) {
      case 1:
        return this._view.getUint8(pointer);
      case 2:
        return this._view.getInt8(pointer);
      case 3:
        return this._view.getUint16(pointer, this._littleEndian);
      case 4:
        return this._view.getInt16(pointer, this._littleEndian);
      case 5:
        return this._view.getUint32(pointer, this._littleEndian);
      case 6:
        return this._view.getInt32(pointer, this._littleEndian);
      case 7:
        return this._view.getBigUint64(pointer, this._littleEndian);
      case 8:
        return this._view.getBigInt64(pointer, this._littleEndian);
      case 9:
        return this._view.getFloat32(pointer, this._littleEndian);
      case 10:
        return this._view.getFloat64(pointer, this._littleEndian);
    }
    return undefined;
  }
  ...
  _write(
    /**@type {number} */ pointer,
    /**@type {number} */ type,
    /**@type {number|bigint} */ value
  ) {
    switch (type) {
      case 1:
        this._view.setUint8(pointer, /**@type {number}*/ (value));
        break;
      case 2:
        this._view.setInt8(pointer, /**@type {number}*/ (value));
        break;
      case 3:
  ...
  _createProperty(
    /**@type {number}*/ propertynamePointer,
    /**@type {number}*/ propertynameLen,
    /**@type {number}*/ offset,
    /**@type {number}*/ viewType,
    /**@type {number}*/ viewLength
  ) {
    this._classUnderConstruction.classInfo.properties.push({
  ...
  createPropertyFromDescriptor(
    /**@type {object}*/ object,
    /**@type {object}*/ descriptor
  ) {
    const basePointer = object?.pointerToInstance ?? 0;
    if (descriptor.viewLength > 1) {
      const buffer = new LinearMemoryBuffer(
        basePointer + descriptor.offset,
        descriptor.viewLength,
        descriptor.viewType,
        this
      );
  ...
  _createStaticProperty(
    ...
    this.createPropertyFromDescriptor(this._classUnderConstruction, {
      name: this.toString(propertynamePointer, propertynameLen),
      offset: pointer,
      viewType: viewType,
      viewLength: viewLength,
  ...
  _createMethod(
    /**@type {number}*/ methodnamePointer,
    /**@type {number}*/ methodnameLen,
    /**@type {number}*/ returnPointerType
  ) {
    const methodname = this.toString(methodnamePointer, methodnameLen);
    this._classUnderConstruction.classInfo.methods.push({
      name: methodname,
      function:
        this[this._classUnderConstruction.classInfo.name + "::" + methodname],
  ...
  _createStaticMethod(
    ...
    const methodname = this.toString(methodnamePointer, methodnameLen),
      wasmMethodname =
        this._classUnderConstruction.classInfo.name + "::" + methodname;
    this._classUnderConstruction[methodname] = this._invokeFunction.bind(
      this,
      0,
      this[wasmMethodname],
      returnPointerType
  ...
  _createFunction(
    ...
    this[methodname] = this._invokeFunction.bind(
      this,
      0,
      this[methodname],
      returnPointerType
  ...
  toString(/**@type {number}*/ pointer, /**@type {number}*/ strlen = 0) {
    ...
    if (strlen < 1) {
      const viewLength = Math.min(
        16384,
        this.linearMemory.byteLength - pointer
      );
      view = new Uint8Array(this.linearMemory, pointer, viewLength);
      for (strlen = 0; strlen < viewLength; strlen++)
        if (view[strlen] == 0) break;
    ...
    while (i < strlen) {
      ...
      if (strlen - i - bytesNeeded > 0) {
        for (let k = 0; k < bytesNeeded; k++)
          codePoint = (codePoint << 6) | (view[i + k + 1] & 0x3f);
      } else {
        codePoint = 0xfffd;
  ...
  toWASMString(
    /**@type {string} */ str,
    /**@type {Uint8Array|undefined}*/ view
  ) {
    ...
    let i = 0,
      ...
      codePoint;
    if (view == undefined)
      view = new Uint8Array(this.linearMemory, this.malloc(maxBytes), maxBytes);
    while (i < length) {
    ...
    if (this.logMemory)
      console.log(
        "WASM memory " +
          this.id +
          ": " +
          this._niceSize(this._stackSize()) +
          " stack, " +
          this._niceSize(this.linearMemory.byteLength - this._heapBase()) +
          " heap, " +
          this._niceSize(this.linearMemory.byteLength) +
          " total."
      );
  }
  ...
  _updateMemoryViews() {
    for (const [pointer, set] of this._buffers)
      for (const buffer of set) buffer.update();
    this._memoryGrowArray = new Uint8Array(
      this.linearMemory,
      this._memoryGrowPointer,
      16
  ...
  addBuffer(/**@type {LinearMemoryBuffer} */ buffer) {
    ...
    if (existing) existing.add(buffer);
    else this._buffers.set(buffer.pointer, new Set([buffer]));
  }

  removeBuffer(/**@type {LinearMemoryBuffer} */ buffer) {
    const set = this._buffers.get(buffer.pointer);
    if (!set) return;
    else set.delete(buffer);
  ...
  bufferToWASM(
    ...
    const arr =
        buffer.constructor === Array ? buffer[index].array : buffer.array,
      to = arr.length;
  ...
  arrayBufferToWASM(
    ...
    new Uint8Array(this.linearMemory).set(
  ...
  copyWASMToArrayBuffer(
    /**@type {number}*/ pointer,
    /**@type {number}*/ lengthBytes
  ) {
    const arrayBuffer = new ArrayBuffer(lengthBytes);
    new Uint8Array(arrayBuffer, 0, lengthBytes).set(
      new Uint8Array(this.linearMemory, pointer, lengthBytes)
    );
    return arrayBuffer;
  }
  ...
  static async loaderWorkerMain(/**@type {string}*/ url) {
    ...
    await fetch(url)
      ...
      .then((audiofileArrayBuffer) => {
        const audiofileInWASMHeap =
          Superpowered.arrayBufferToWASM(audiofileArrayBuffer);

        const metaData = {};
        // Constructor. Has no additional parameters.
        let metaDecoder = new Superpowered["Decoder"]();

        // Opens a memory location for decoding.
        let openErrorCode = metaDecoder.openAudioFileInMemory(
          audiofileInWASMHeap, // Pointer to an audio file loaded onto the WebAssembly Linear Memory.
          audiofileArrayBuffer.byteLength, // The audio file length in bytes.
          true // If true, it opens the file for fast metadata reading only, not for decoding audio.
        );
        if (openErrorCode === 0) {
          metaData.durationSeconds = metaDecoder.getDurationSeconds();
          // metaData.sampleRate = metaDecoder.getSampleRate();
        }

        const audioInMemoryFormat = Superpowered[
          "Decoder"
        ...
        const arrayBuffer = Superpowered.moveWASMToArrayBuffer(
          audioInMemoryFormat,
          48 + Superpowered["AudioInMemory"].getSize(audioInMemoryFormat) * 4
        );
        postMessage({ __transfer__: arrayBuffer, metaData: metaData }, [
          arrayBuffer,
        ]);
      });
  }
  ...
  static loaderWorkerOnmessage(/**@type {MessageEvent}*/ message) {
    if (typeof message.data.load === "string")
      SuperpoweredGlue.loaderWorkerMain(message.data.load);
  }

  /**@returns {number} */
  registerTrackLoader(/**@type {object}*/ receiver) {
    if (typeof receiver.terminate !== "undefined")
      receiver.addEventListener("message", this.handleTrackLoaderMessage); // Worker
    this._trackLoaderReceivers.set(
      this._nextTrackLoaderReceiverID++,
      typeof receiver.port !== "undefined" ? receiver.port : receiver
    );
    return this._nextTrackLoaderReceiverID - 1;
  }

  removeTrackLoader(/**@type {number} */ trackLoaderID) {
    this._trackLoaderReceivers.delete(trackLoaderID);
  }
  /**@returns {number} */ nextTrackLoaderID() {
    return this._nextTrackLoaderReceiverID;
  }

  handleTrackLoaderMessage(/**@type {MessageEvent}*/ message) {
    if (typeof message.data.SuperpoweredLoad !== "string") return false;
    this.loadTrackInWorker(
      message.data.SuperpoweredLoad,
      message.data.trackLoaderID
    );
    return true;
  }

  async loadTrackInWorker(
    /**@type {string}*/ url,
    /**@type {number}*/ trackLoaderID
  ) {
    if (this._trackLoaderSource == undefined)
      this._trackLoaderSource = URL.createObjectURL(
        new Blob(
          [
            SuperpoweredGlue.toString() +
              "\r\n\r\nonmessage = SuperpoweredGlue.loaderWorkerOnmessage;" +
              `\r\n\r\nSuperpoweredGlue.wasmCDNUrl = "${SuperpoweredGlue.wasmCDNUrl}";`,
          ],
    ...
    trackLoaderWorker.onmessage = (/**@type {MessageEvent}*/ message) => {
      console.log(message);
      return this.transferLoadedTrack(
        message.data.__transfer__,
  ...
  transferLoadedTrack(
    ...
    /**@type {Worker} */ trackLoaderWorker
  ) {
    const receiver = this._trackLoaderReceivers.get(
      trackLoaderWorker["trackLoaderID"]
    );
  ...
  downloadAndDecode(/**@type {string}*/ url, /**@type {object}*/ obj) {
    ...
    if (
      typeof obj.onMessageFromMainScope === "function" &&
      typeof obj.sendMessageToMainScope === "function"
    )
      obj.sendMessageToMainScope({
...
class SuperpoweredWebAudio {
  ...
  getUserMediaForAudio(
    ...
    /**@type {(stream:MediaStream)=>void}*/ onPermissionGranted,
    /**@type {(reason:any)=>void}*/ onPermissionDenied
  ) {
    ...
    try {
      navigator.mediaDevices
        .getUserMedia(/**@type {MediaStreamConstraints}*/ (finalConstraints))
  ...
  async createAudioNodeAsync(
    ...
    /**@type {Function}*/ onMessageFromAudioScope,
    /**@type {number}*/ numInputs = 1,
    /**@type {number}*/ numOutputs = 1
  ) {
    return new Promise((resolve, reject) =>
  ...
  createAudioNode(
    /**@type {string}*/ url,
    /**@type {string}*/ className,
    /**@type {(node:AudioWorkletNode)=>void}*/ callback,
    /**@type {Function}*/ onMessageFromAudioScope,
    /**@type {number}*/ numInputs = 1,
    /**@type {number}*/ numOutputs = 1
  ) {
...
if (typeof AudioWorkletProcessor === "function") {
  ...
  class SuperpoweredAudioWorkletProcessor extends AudioWorkletProcessor {
    ...
    constructor(/**@type {object}*/ options) {
      super();
      SuperpoweredGlue["__uint_max__sp__"] =
    ...
    afterWASMLoaded() {
      ...
      for (let n = this.numberOfInputs; n > 0; n--)
        this.inputBuffers.push(new this.Superpowered.Float32Buffer(128 * 2));
      for (let n = this.numberOfOutputs; n > 0; n--)
        this.outputBuffers.push(new this.Superpowered.Float32Buffer(128 * 2));
      this.onReady();
      //@ts-ignore
    ...
    //@ts-ignore
    sendMessageToMainScope(/**@type {any}*/ message) {
      this.port.postMessage(message);
    }
    processAudio(
      /** @type {object|object[]} */ input,
      /** @type {object|object[]} */ output,
      /**@type {number} */ numFrames,
      /**@type {Object<string,Float32Array>} */ parameters
    ) {}
    process(
      /**@type {Float32Array[][]} */ inputs,
      /**@type {Float32Array[][]} */ outputs,
      /**@type {Object<string,Float32Array>} */ parameters
    ) {
      if (this.state < 0) {
        console.log("singalled to end processor");
        return false;
      }
      if (this.state == 1) {
        for (let n = this.numberOfInputs - 1; n >= 0; n--) {
          if (inputs[n].length > 1)
            this.Superpowered.bufferToWASM(this.inputBuffers, inputs, n);
          else
            this.Superpowered["memorySet"](
              this.inputBuffers[n].pointer,
              0,
              128 * 8
            );
        }
        this.processAudio(
          ...
          this.numberOfOutputs == 1
            ? this.outputBuffers[0]
            : this.outputBuffers,
          128,
          parameters
        );
        for (let n = this.numberOfOutputs - 1; n >= 0; n--) {
          if (outputs[n].length > 1)
            this.Superpowered.bufferToJS(this.outputBuffers, outputs, n);
        }
      }
      return true;
    }
  ...
  SuperpoweredWebAudio.AudioWorkletProcessor =
    SuperpoweredAudioWorkletProcessor;
}
