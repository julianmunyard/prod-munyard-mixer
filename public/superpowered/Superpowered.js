...
class SuperpoweredGlue {
    static wasmCDNUrl = '/superpowered/superpowered.wasm';
    ...
    async loadTrackInWorker(/**@type {string}*/url, /**@type {number}*/trackLoaderID) {   
        if (this._trackLoaderSource == undefined) {
            const wasmUrl = (typeof window !== 'undefined' && window.location) ? 
                new URL(SuperpoweredGlue.wasmCDNUrl, window.location.origin).toString() : 
                SuperpoweredGlue.wasmCDNUrl;
            this._trackLoaderSource = URL.createObjectURL(new Blob([ SuperpoweredGlue.toString() + "\r\n\r\nonmessage = SuperpoweredGlue.loaderWorkerOnmessage;" + `\r\n\r\nSuperpoweredGlue.wasmCDNUrl = "${wasmUrl}";` ], { type: 'application/javascript' }));
        }
...
// Also set global variables for UMD compatibility
